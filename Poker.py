...
class ComputerPlayer:
    ...
    def bet_strategy(self, board, need):
        ...
        # elif large number on card, still high chance but lower than pairs
        elif hand_num in [("A", "A"), ("A", "K"), ("Q", "Q")]:
            fold_low= 0.2

        # elif player only hold small cards
        elif hand_num[0] in range(1, 9) and hand_num[1] in range(1, 9):
            fold_low = 0.6
        ...
        if fold_low:
            rate_fold = random.random()
            if rate_fold < fold_low:
                fold = True
            else:
                bet_amount = need + self.chips * random.random()
    ...
    def decision(self, stage, pod, highbet, board):
        ...
        if self.alive:
                ...
                if stage == 'preflop': #preflop round
                    if self.blind:  #bet if SBlind/BBlind
                        if self.seat == 'SB':
                            ...
                            # if need random 
                            fold, bet = self.bet_strategy(board, needed)
            ...
            return [bet, self.bet, self.allin] 
